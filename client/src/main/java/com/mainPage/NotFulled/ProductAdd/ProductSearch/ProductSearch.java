...
import com.mainPage.NotFulled.ProductAdd.DictionarySearchType;
...
public class ProductSearch extends GridComp implements ProductSearchInterface {
  ...
  /*  public void setID(String ID) {
    ...
    public List findByProperty(Object value, Enum<? extends SearchType> searchType) {
        ...
        switch ((DictionarySearchType) searchType) {
            case ID:
                whereClause = " WHERE \n" +
                                " [tbl_Offering].ID = ?";
            ...
            case NAME:
                whereClause = " WHERE \n" +
                        "  [tbl_Offering].[Name] LIKE ?";
                valueClause = "%" + value.toString() + "%";
                break;
            default:
                log.log(Level.SEVERE, "Valid search account type");
        }
                try (Connection connection = DBConnection.getDataSource().getConnection()) {
                    list = FXCollections.observableArrayList(dbAccess.query(connection, queries.getMainProductSearch(false, 0) + whereClause, new BeanListHandler<ProductSearch>(ProductSearch.class), valueClause));
                } catch (Exception e) {
        ...
        return (list.isEmpty() ? FXCollections.observableArrayList(EMPTYLIST) : list);
    }
    @Override
