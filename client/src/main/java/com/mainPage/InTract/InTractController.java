...
public class InTractController extends WorkArea  implements MiniFilterFunction, Initializable, DictionaryProperties, ObserverNF {
    ...
    @FXML
    public JFXToggleButton btn_ButtonAll;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        optionsStatusRequest.addAll(btnConfirmation.getText(),btn_Delete.getText(), btn_СancelRequest.getText(), btn_Create.getText(), btn_Refresh.getText(), btn_ButtonAll.getText(), notNotification.getText());
    ...
    public void findByProperty(String value) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                List<InTract> listItems = account.findSearchSkrut( Skrut);
   ...
   public static String  serverName = getServerName();
    ...
    public void loginButtonAction(InTract chosenAccount) throws IOException {
        ...
        Listener listener = new Listener(chatViewController, chosenAccount);
        Listener listenerConnect = new Listener();
        if(listenerConnect.socket == null) {
    ...
    public BorderPane createPage(int pageIndex) {
        try {
            ...
            if(queryAll == false) {
                ...
                toIndex = Math.min(fromIndex + itemsPerPage, accountQueries.getMainInTractCount());

                pageCount = getPageCount((int) accountQueries.getMainInTractCount(), itemsPerPage);
