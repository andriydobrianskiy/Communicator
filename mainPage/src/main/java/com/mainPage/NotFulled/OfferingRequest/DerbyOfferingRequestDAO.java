...
public class DerbyOfferingRequestDAO implements OfferingRequestDAO{
    ...
    public List<OfferingRequest> findAll(int rowIndex, String requestID) {
        try {
            return dbAccess.query(DBConnection.getDataSource().getConnection(), queries.getOfferingRequestQuery(true,10,requestID ), new BeanListHandler<>(OfferingRequest.class));
