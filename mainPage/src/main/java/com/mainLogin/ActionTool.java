...
public final class ActionTool {
  ...
  //  private static ImageView questionImage = InfoTool.getImageViewFromResourcesFolder("question.png");
    ...
    public static void showNotification(String title , String text , Duration duration , NotificationType notificationType , Image image , int width , int height) {
        ...
        if (image == null)
            notification1 = Notifications.create().title(title).text(text).hideAfter(duration).darkStyle().position(notificationPosition);
        else {
    ...
    public static boolean doQuestion(String headerText , String text , Node node , Stage window) {
     ...
     //   alert.setGraphic(questionImage);
        ...
        alert.heightProperty().addListener(l -> {
            ...
            if (x + alertWidth > UsefulUtils.getVisualScreenWidth())
                x = (int) ( UsefulUtils.getVisualScreenWidth() - alertWidth );
            else if (x < 0)
                x = 0;

            // Check if Alert goes out of the Screen on Y AXIS
            if (y + alertHeight > UsefulUtils.getVisualScreenHeight())
                y = (int) ( UsefulUtils.getVisualScreenHeight() - alertHeight );
            else if (y < 0)
                y = 0;
